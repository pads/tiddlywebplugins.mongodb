"""
Adapted from the tiddlyweb text store tests.
Probably need to change these to an adaptation of the sqlalchemy gamut tests
"""
import py.test
...
from tiddlyweb.store import NoTiddlerError, NoUserError


def setup_module(module):
    module.store = get_store(config)
    client = MongoClient(config['server_store'][1]['db_config'])
    client.drop_database('tiddlyweb')
...
def test_users():
    ...
    assert len(users) == 2
    assert ['cdent', 'pads'] == sorted([user.usersign for user in users])

    store.delete(User('cdent'))

    users = list(store.list_users())
    assert len(users) == 1

    py.test.raises(NoUserError, "store.get(User('cdent'))")
...
def test_recipes():
    ...
    recipec.policy.accept.append('pads')
    recipec.set_recipe([
